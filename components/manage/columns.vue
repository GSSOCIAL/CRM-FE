...
const columnManagementMouseDownEvent = (event: MouseEvent) => {
  ...
  if (target != null && target.closest(".manageColumnItem")) {
    ...
    columnsHandlers.originalColumn =
      target.closest(".manageColumnItem")!.dataset["prop"];
    columnsHandlers.originalList = target
      .closest(".manageColumnsList")
      ?.classList.contains("listForSelectedColumns")
      ? "selected"
      : "available";
    ...
    columnsHandlers.clone.innerText =
      columns[
        columns.findIndex(({ prop }) => prop == columnsHandlers.originalColumn)
      ]?.label;
...
const columnManagementMouseMoveEvent = (event) => {
  if (columnsHandlers.isDragMode && columnsHandlers.clone) {
    ...
    let selectedColumnIndex = columns.findIndex(
      ({ prop }) => prop == columnsHandlers.originalColumn
    );
    ...
    if (elements && elements.length > 0) {
      ...
      let index = elements.findIndex(
        ({ className }) => className == "manageColumnItem"
      );
      if (index >= 0) {
        //Look for detected element
        let columnIndex = columns.findIndex(
          ({ prop }) => prop == elements[index].dataset["prop"]
        );

        //Next - if drag available column into available list - do nothing
        if (
          columnsHandlers.originalList == "available" &&
          elements[index].closest(".availableColumnsList")
        ) {
        ...
        if (columnIndex >= 0) {
          ...
          if (
            typeof columns[selectedColumnIndex].isDisplayed == "undefined" ||
            columns[selectedColumnIndex].isDisplayed == false
          ) {
            let updateColumns = columns;
            ...
            updateColumns = updateColumns.map((col) => {
              ...
              if (inSelectedListIndex >= 0) {
                return Object.assign(col, {
                  order:
                    inSelectedListIndex <= overChildIndex
                      ? inSelectedListIndex
                      : inSelectedListIndex + 1,
                });
              }
            });
    ...
    let updateColumns = columns;
    updateColumns[selectedColumnIndex].isDisplayed = !(
      columnsHandlers.originalList == "available"
    );
    columns.value = updateColumns;
  }
